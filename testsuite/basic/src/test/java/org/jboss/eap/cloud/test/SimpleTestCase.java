...
public class SimpleTestCase {
    ...
    public void test() throws Exception {
        ...
        if (bootableJAR) {
            ...
            // Check that mp-config.sh has been included in the set of script files
            Path mpConfig = install.resolve("bin/launch/mp-config.sh");
            assertTrue(Files.exists(mpConfig));
            Path launchConfig = install.resolve("bin/launch/launch-config.sh");
            assertTrue(Files.exists(launchConfig));
            String content = new String(Files.readAllBytes(launchConfig));
            assertTrue(content.contains("$JBOSS_HOME/bin/launch/mp-config.sh"));
        }
    }
}